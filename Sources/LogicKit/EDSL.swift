...
extension Term {
    ...
    public func extractValue<T>(ofType type: T.Type) -> T? {
        guard case .val(let v) = self else { return nil }
        return v as? T
    }
    ...
    public subscript(terms: Term...) -> Term {
        guard case ._term(let name, let subterms) = self, subterms.isEmpty else { fatalError("Cannot coerce '\(self)' into a functor") }
